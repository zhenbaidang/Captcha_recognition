...
def get_data_split(path, split=[6, 1, 1], save=True, out_dir='./data', modes=['train', 'dev', 'test']):
    ...
    if not os.path.exists(out_dir):
        os.mkdir(out_dir)
        train_path, dev_path, test_path = os.path.join(out_dir, 'train.npy'), os.path.join(
            out_dir, 'dev.npy'), os.path.join(out_dir, 'test.npy')
    if os.path.exists(train_path) and os.path.exists(dev_path) and os.path.exists(test_path):

        if 'train' in modes:
            x_train, y_train = np.load(train_path, allow_pickle=True), np.load(
                os.path.join(out_dir, 'train.y.npy'), allow_pickle=True)
        if 'dev' in modes:
            x_dev, y_dev = np.load(dev_path, allow_pickle=True), np.load(
                os.path.join(out_dir, 'dev.y.npy'))
        if 'test' in modes:
            x_test, y_test = np.load(test_path, allow_pickle=True), np.load(
                os.path.join(out_dir, 'test.y.npy'))
        ...
        x_train, x_dev_test, y_train, y_dev_test = train_test_split(
            names, labels, train_size=ratios[0])
        ratios = np.array(split[1:]) / np.sum(split[1:])
        x_dev, x_test, y_dev, y_test = train_test_split(
            x_dev_test, y_dev_test, train_size=ratios[0])

        if save:
            np.save(train_path, x_train, allow_pickle=True)
            np.save(os.path.join(out_dir, 'train.y.npy'),
                    y_train, allow_pickle=True)
            np.save(dev_path, x_dev, allow_pickle=True)
            np.save(os.path.join(out_dir, 'dev.y.npy'),
                    y_dev, allow_pickle=True)
            np.save(test_path, x_test, allow_pickle=True)
            np.save(os.path.join(out_dir, 'test.y.npy'),
